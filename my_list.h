...
public:																					
													//added by nader
	std::vector<int> indicesVisited;				//will check which indices have already been visted												
	int sizeList;									//holds the size if the list
	...
	void moveIterator(int steps_to_move) {												//added by nader (remove)
		if (steps_to_move > 0) {												//this means we have to move backwards
			for (int i = 0; i < steps_to_move; i++)
				this->_Ptr = this->_Ptr->_Prev;
		}
		if (steps_to_move < 0) {												//this means we have to move forward
			for (int i = 0; i < -steps_to_move; i++)
				this->_Ptr = this->_Ptr->_Next;
		}
	}

	_List_const_iterator operator++(int)												//added by nader (remove)
	{	// postincrement
		if (indicesVisited.size() == sizeList) {											//if all indices were visited, move to last element
																							//in order for iterator to become equal to list.end()
			int last_index = indicesVisited.at(indicesVisited.size() - 1);					//get the index of the last element
			int last_movement = last_index - (sizeList - 1);								//this will be the number of steps to move in order to reach the last element
			moveIterator(last_movement);													//move the iterator
			this->_Ptr = this->_Ptr->_Next;													//this is necessary in order for iterator to become equal to list.end()
			return *this;
		}

		srand(time(NULL));
		int visitNow = rand() % sizeList;													//get the index to visit in this iteration
		bool doesContain;																	//will check if the index visitNow was already visited
		if (std::find(indicesVisited.begin(), indicesVisited.end(), visitNow) != indicesVisited.end())		//checking if visitNow was already visited
			doesContain = true;
		else
			doesContain = false;

		while (doesContain == true) {														//this while loop will attempt to generate an index that has not been generated yet
			visitNow = rand() % sizeList;													//generate new index
			if (!(std::find(indicesVisited.begin(), indicesVisited.end(), visitNow) != indicesVisited.end()))		//check if this index already visited
			{
				// Element in vector.
				doesContain = false;
			}
...
public:
	...
	_List_const_iterator<_Mylist> LastElement;
...
#pragma warning(pop)
	...
	const_iterator mybegin() _NOEXCEPT
	{	// return iterator for beginning of mutable sequence
																									//added by nader
		const_iterator tempit = const_iterator(this->_Myhead()->_Next, _STD addressof(this->_Get_data()));		//create an iterator at the start of the list					
